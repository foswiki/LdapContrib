...
use Net::LDAP::Constant
  qw(LDAP_SUCCESS LDAP_SIZELIMIT_EXCEEDED LDAP_CONTROL_PAGED);
...
sub new {
    ...
    my $this = {
        ...
        loginFilter => $Foswiki::cfg{Ldap}{LoginFilter}
          || 'objectClass=posixAccount',

        groupAttribute => $Foswiki::cfg{Ldap}{GroupAttribute}
          || 'cn',
        primaryGroupAttribute => $Foswiki::cfg{Ldap}{PrimaryGroupAttribute}
          || 'gidNumber',
        groupFilter => $Foswiki::cfg{Ldap}{GroupFilter}
          || 'objectClass=posixGroup',
        memberAttribute => $Foswiki::cfg{Ldap}{MemberAttribute}
          || 'memberUid',
        innerGroupAttribute => $Foswiki::cfg{Ldap}{InnerGroupAttribute}
          || 'uniquegroup',
        memberIndirection => $Foswiki::cfg{Ldap}{MemberIndirection}
          || 0,
        nativeGroupsBackoff => $Foswiki::cfg{Ldap}{WikiGroupsBackoff}
          || 0,
        bindDN => $Foswiki::cfg{Ldap}{BindDN}
          || '',
        bindPassword => $Foswiki::cfg{Ldap}{BindPassword}
          || '',
        mapGroups => $Foswiki::cfg{Ldap}{MapGroups}
          || 0,
        rewriteGroups => $Foswiki::cfg{Ldap}{RewriteGroups}
          || {},
        rewriteWikiNames => $Foswiki::cfg{Ldap}{RewriteWikiNames}
          || {},
        mergeGroups => $Foswiki::cfg{Ldap}{MergeGroups}
          || 0,
        ...
        saslMechanism => $Foswiki::cfg{Ldap}{SASLMechanism}
          || 'PLAIN CRAM-MD4 EXTERNAL ANONYMOUS',
    ...
    @{ $this->{wikiNameAttributes} } =
      split( /\s*,\s*/, $this->{wikiNameAttribute} );
...
sub connect {
    ...
    # proxy user
        ...
        if ( $this->{useSASL} ) {
            ...
            $msg =
              $this->{ldap}
              ->bind( $this->{bindDN}, password => $this->{bindPassword} );
        }
    }
...
sub getAccount {
    ...
    my $filter =
      '(&' . $loginFilter . '(' . $this->{loginAttribute} . '=' . $login . '))';
    my $msg = $this->search(
        filter => $filter,
        base   => $this->{userBase}
    );
...
sub search {
    ...
    if ( $Foswiki::cfg{Ldap}{Debug} ) {
        ...
        writeDebug(
"called search(filter=$args{filter}, base=$args{base}, scope=$args{scope}, sizelimit=$args{sizelimit}, attrs=$attrString)"
        );
    }
...
sub initCache {
    ...
    $this->{cacheDB} = tie %{ $this->{data} }, 'DB_File', $this->{cacheFile},
    ...
    if ( $this->{maxCacheAge} > 0 ) {    # is cache expiration enabled
        ...
        writeDebug(
"cacheAge=$cacheAge, maxCacheAge=$this->{maxCacheAge}, lastUpdate=$lastUpdate, refresh=$refresh"
        );
    }
...
sub refreshCache {
    ...
    my %tempData;
    my $tempCache = tie %tempData, 'DB_File', $tempCacheFile, O_CREAT | O_RDWR,
      0664, $DB_HASH
    ...
    if ( $this->{preCache} ) {
        ...
        unless ($isOk)
        {    # we had an error: keep the old cache til the error is resolved
            undef $tempCache;
            untie %tempData;
...
sub refreshUsersCache {
    ...
    my @args = (
        ...
        attrs  => [
            $this->{loginAttribute},        $this->{mailAttribute},
            $this->{primaryGroupAttribute}, @{ $this->{wikiNameAttributes} }
        ],
    );
...
# use the control LDAP extension only if a valid pageSize value has been provided
    ...
    while (1) {
        ...
        unless ($mesg) {
            writeWarning(
                "error refreshing the user cache: " . $this->getError() );
            $gotError = 1
              unless $this->getCode() ==
                  LDAP_SIZELIMIT_EXCEEDED;    # continue on sizelimit exceeded
            last;
        }

        # process each entry on a page
        while ( my $entry = $mesg->pop_entry() ) {
            $this->cacheUserFromEntry( $entry, $data, \%wikiNames,
                \%loginNames )
              && $nrRecords++;
        }
...
sub resolveWikiNameClashes {
    ...
    foreach my $item ( sort { $a->{loginName} cmp $b->{loginName} }
        values %{ $this->{_wikiNameClaches} } )
    {
        ...
        my $dn        = $item->{dn};
        my $prevWikiName =
          ( $refreshMode < 2 ) ? $this->getWikiNameOfDn($dn) : '';
        my $newWikiName;
        my $prevDN;
        ...
        writeDebug(
"processing clash of loginName=$loginName on wikiName=$wikiName, dn=$item->{dn}, resolves to newWikiName=$newWikiName"
        );

        $this->cacheUserFromEntry( $item->{entry}, $data, $wikiNames,
            $loginNames, $newWikiName );
        $nrRecords++;
    }
...
sub refreshGroupsCache {
    ...
    my @args = (
        ...
        attrs  => [
            $this->{groupAttribute},      $this->{memberAttribute},
            $this->{innerGroupAttribute}, $this->{primaryGroupAttribute}
        ],
    );
...
# use the control LDAP extension only if a valid pageSize value has been provided
    ...
    while (1) {
        ...
        unless ($mesg) {
            ...
            writeWarning(
                "error refeshing the groups cache: " . $this->getError() );
            last;
        }
       ...
       # have to lookup the corresponding loginAttribute
            if ( $this->{memberIndirection} ) {
                ...
                if ($memberName) {
                    ...
                    writeWarning(
                        "oops, $member not found, but member of $groupName");
                }
            }
            else {
                $members{$member} = 1;
    ...
    if ($isExplicitWikiName) {
        ...
        if ($prevWikiName) {
            ...
            unless ($wikiName) {
                $wikiName = $loginName;
                writeWarning(
"no WikiNameAttributes found for $dn ... deriving WikiName from LoginName: '$wikiName'"
                );
            }
            ...
            foreach my $pattern ( keys %{ $this->{rewriteWikiNames} } ) {
                ...
                if ( $wikiName =~ /^(?:$pattern)$/ ) {
                    ...
                    $wikiName = $subst;
                    writeDebug(
"rewriting '$oldWikiName' to '$wikiName' using rule $pattern"
                    );
                    last;
                }
            }
    ...
    # 5. check if this dn maps to a wikiName already in use by another dn before
            ...
            if ( $prevDN && $prevDN ne $dn ) {
                ...
                writeWarning(
"$dn clashes with wikiName $wikiName already in use by $prevDN ... renaming later"
                );
            ...
            if ( defined $clashDN ) {
                if ( $dn ne $clashDN ) {
                    writeWarning(
"$dn clashes with $clashDN on wikiName $wikiName ... renaming later"
                    );
                    $this->{_wikiNameClaches}{$dn} = {
                        entry     => $entry,
                        dn        => $dn,
                    ...
                    writeWarning(
                        "$dn found twice in ldap search... ignoring second one"
                    );
                }
                return 0;
            }
    ...
    if ( defined( $loginNames->{$loginName} ) ) {
        ...
        writeWarning(
"$dn clashes with $clashDN on loginName $loginName ... please configure a unique loginName attribute"
        );
        return 0;
    }
    ...
    foreach my $groupName ( keys %groupNames ) {
        if ( defined $data->{"GROUP2UNCACHEDMEMBERSDN::$groupName"} ) {
            my $dnList = Foswiki::Sandbox::untaintUnchecked(
                $data->{"GROUP2UNCACHEDMEMBERSDN::$groupName"} )
              || '';
          ...
          LOOP: {
                foreach my $memberDn (@membersDn) {
                    if ( $memberDn eq $dn ) {
                        ...
                        writeDebug(
                            "refreshing group $groupName to catch new members");
...
sub cacheGroupFromEntry {
    ...
    foreach my $pattern ( keys %{ $this->{rewriteGroups} } ) {
        ...
        if ( $groupName =~ /^(?:$pattern)$/ ) {
            ...
            $foundRewriteRule = 1;
            writeDebug(
                "rewriting '$oldGroupName' to '$groupName' using rule $pattern"
            );
            last;
        }
    }
    ...
    if ( !$this->{mergeGroups} && defined( $groupNames->{$groupName} ) ) {
        writeWarning(
            "$dn clashes with group $groupNames->{$groupName} on $groupName");
        return 0;
    }

    if (   defined( $data->{"U2W::$groupName"} )
        || defined( $data->{"W2U::$groupName"} ) )
    {
        my $groupSuffix = '';
        if ( $this->{normalizeGroupName} ) {
            $groupSuffix = 'Group';
...
#writeWarning("group $dn clashes with user $groupName ... appending $groupSuffix");
    ...
    foreach my $member ( $entry->get_value( $this->{memberAttribute} ) ) {
        ...
        $member =~ s/^\s+//o;
        $member =~ s/\s+$//o;
        $this->{_groups}{$groupName}{$member} =
          1;    # delay til all groups have been fetched
    }
    ...
    foreach my $innerGroup ( $entry->get_value( $this->{innerGroupAttribute} ) )
    {
        ...
        $innerGroup =~ s/^\s+//o;
        $innerGroup =~ s/\s+$//o;
        $this->{_groups}{$groupName}{$innerGroup} =
          1;    # delay til all groups have been fetched
    }
...
# create a proper WikiName
    if (   !Foswiki::Func::isValidWikiWord($wikiName)
...
sub getEmails {
    ...
    my $emails =
      Foswiki::Sandbox::untaintUnchecked( $data->{ "U2EMAIL::" . $login } )
      || '';
...
sub getLoginOfEmail {
    ...
    my $loginNames =
      Foswiki::Sandbox::untaintUnchecked( $data->{ "EMAIL2U::" . $email } )
      || '';
...
# Make sure that the group is in the cache. This will cause the addition of the group to the cache if it exists in LDAP
    ...
    my $members =
      Foswiki::Sandbox::untaintUnchecked( $data->{"GROUPS::$groupName"} ) || '';
...
sub getLoginOfWikiName {
    ...
    my $loginName =
      Foswiki::Sandbox::untaintUnchecked( $data->{"W2U::$wikiName"} );

    unless ($loginName) {
        my $alias = $this->{wikiNameAliases}{$wikiName};
        $loginName =
          Foswiki::Sandbox::untaintUnchecked( $data->{"W2U::$alias"} )
          if defined($alias);
    }
...
sub changePassword {
    ...
    return undef
      unless $this->{allowChangePassword}
          && defined($oldPassword)
          && $oldPassword ne '1';
...
sub removeUserFromCache {
    ...
    foreach my $email ( @{ $this->getEmails( $loginName, $data ) } ) {
        my %emails =
          map { $_ => 1 } split( /\s*,\s*/, $data->{"EMAIL2U::$email"} );
...
# Skip lookup if group was already not found in LDAP since last cache expiration
    ...
    # update cache selectively
    writeDebug(
        "group $groupName is unknown, need to refresh part of the ldap cache");
    ...
    unless ($entry) {
        ...
        if ( $this->{_primaryGroup} ) {
            foreach my $groupId ( keys %{ $this->{_primaryGroup} } ) {
                ...
                if ( defined $currentGroupName
                    && $groupName eq $currentGroupName )
                {
                    foreach
                      my $member ( keys %{ $this->{_primaryGroup}{$groupId} } )
                    {
...
# Smell: this may not be reliable and may work only with membersindirection. TO CHECK
                        if (   $innerGroupName ne ""
                            && $this->isGroup( $innerGroupName, $data ) )
                        {
    ...
    if ( $] < 5.008 ) {
        ...
        if ( Unicode::MapUTF8::utf8_supported_charset($charset) ) {
            return Unicode::MapUTF8::from_utf8(
                { -string => $utf8string, -charset => $charset } );
        }
        else {
            $this->writeWarning( 'Conversion from $encoding no supported, '
